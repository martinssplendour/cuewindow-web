(function () {
  ...
  function functionUrl(name) {
    const base = String(state.config.supabaseUrl || "").replace(/\/+$/, "");
  ...
  async function startCheckout(plan, button) {
    if (!supabaseConfigured()) return showNotice("Billing is not configured for this website yet.", "error");
    if (!state.session?.accessToken) {
      showNotice("Sign in before upgrading.", "error");
    ...
    try {
      ...
      const response = await fetch(functionUrl("stripe-checkout"), {
        method: "POST",
        headers: userHeaders(),
        body: JSON.stringify({
          plan,
      ...
      const data = await parseResponse(response, "Could not start checkout");
  ...
  function boot() {
    ...
    document.querySelectorAll("[data-checkout-plan]").forEach((button) => {
      button.addEventListener("click", () => startCheckout(button.dataset.checkoutPlan, button));
    });
